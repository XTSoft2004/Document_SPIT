...
export default function GridDocument({
  data,
  content,
  slug,
  path,
  ...
  const getCurrentfolderId = useCallback(() => {
    ...
    for (const pathSegment of path) {
      const found = current.find(
        (item) => item.name === pathSegment && item.isFolder,
      )
      if (found) {
    ...
    <div className="flex h-full min-h-0 relative">
      ...
      <div className="flex-1 flex flex-col min-h-0 relative overflow-hidden">
        <div className="flex flex-col xs:flex-row xs:items-center xs:justify-between mb-3 sm:mb-4 w-full gap-2 xs:gap-3 flex-shrink-0 pt-3 sm:pt-4 px-3 sm:px-4">
          <div className="flex flex-col sm:flex-row flex-1 min-w-0 overflow-hidden gap-2">
          ...
          <div className="flex gap-1 xs:gap-2 items-center justify-end flex-shrink-0">
            ...
            <div className="hidden xs:flex gap-1">
              <button
                className={`relative overflow-hidden p-1.5 xs:p-2 rounded transition-all duration-200 ${
                  mode === 'list'
              ...
              <button
                className={`relative overflow-hidden p-1.5 xs:p-2 md:mr-5 rounded transition-all duration-200 ${
                  mode === 'preview'
                    ? 'bg-blue-100 text-blue-600 scale-110 shadow'
                    : 'bg-gray-200 text-gray-500 scale-100'
                }`}
            ...
            onClick={() => {
              if (path.length > 0) {
                const parentUrl =
                  path.length > 1
                    ? '/document/' +
                      path.slice(0, -1).map(convertSlug).join('/')
                    : '/document'
