  ...
  const columns: TableColumnType<IUserResponse>[] = [
    ...
    },
    {
      ...
      render: (text: string | undefined) =>
        text && text.trim() !== '' ? text : 'Không có tên',
    },
    {
      title: 'Role',
      dataIndex: 'roleName',
      ...
      render: (roleName: string | undefined, record: IUserResponse) => (
        <select
          ...
          onChange={async (e) => {
            ...
            const setRole = await setRoleUser(record.username, newRole)
            if (setRole.ok) {
              reloadTable('user')
        ...
        >
          <option
            value="Admin"
            className="text-[#f87171] font-semibold text-left"
          >
            🛡️ Quản trị viên
          </option>
      ...
      render: (islocked: boolean, record: IUserResponse) => (
        <select
          ...
          onChange={async (e) => {
            ...
            if (banAccount.ok) {
              NotificationService.success({
                message: `Tài khoản ${record.fullname} đã ${
        ...
        >
          <option
            value="unlocked"
            className="text-[#16a34a] font-medium text-left"
          >
            🔓 Hoạt động
          </option>
          <option
            value="locked"
            className="text-[#dc2626] font-semibold text-left"
          >
            🔒 Bị khoá
          </option>
        </select>
